...
| `avstamning/` | — | **Paket**, samma form som `bokslutskontroll/`. Lager 1b i `hantverksbok/BOKSLUTSPROGRAMMET.md` §4 — kontoutdragsavstämning mot en utomstående källa (camt.053 + CSV), delar `Fynd`/motor/register med lager 1. Steg 1–5 av 6 klara: `camt053.py`, `csvprofil.py`, `matchning.py` (fyra pass, §4.3.1:s parkoppling ger A-03), `kontroller.py` (A-01–A-05, registrerade i samma `bokslutskontroll.motor.KONTROLLER`), `sekretess.py` (maskering, samma `_maskera_fritext`-primitiv som `maskera_siefil`; sökvägsvakten `_tillaten_kontoutdrag` bor i `mcp_server/server.py`). Steg 6 (🏦-knappen i bokslutsrummet) återstår — inget MCP-verktyg är specat för lager 1b. Beroendet går bara åt ett håll: `avstamning` importerar `bokslutskontroll`, aldrig tvärtom. | Under uppbyggnad |
