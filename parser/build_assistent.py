...
# Combine AI Settings and Chat Logic into parser/assistent.py
with open("parser/ai_sidebar_lines.py", "r", encoding="utf-8") as f:
    sidebar_lines = f.read()

with open("parser/ai_assistent_lines.py", "r", encoding="utf-8") as f:
    chat_lines = f.read()

assistent_content = """import streamlit as st
...
import sessionslogg
from ai_konfiguration import AIKonfiguration, LEVERANTÖRER, uppdatera_med_hamtade_modeller, leverantor_har_analysstod, leverantor_har_samtalsstod
...
from samtalsflode import (
    ...
    bygg_chattanropare, bygg_agentanropare, SamtalanropareFel
)
...
from verktyg_val import tolka_valverktygsanrop
from verktyg_skapa_faktura import tolka_fakturaverktygsanrop
from verktyg_skapa_kund import tolka_kundverktygsanrop
...
def rendera_panel(st, rum_id: str, kontext: AssistentKontext) -> None:
    ...
    with st.expander("💬 Fråga om det här", expanded=False):
        ...
        if ai_konfiguration is None or ai_konfiguration.status != "modeller_hämtade":
            st.info("AI-modeller saknas. Fyll i inställningarna ovan.")
            return
        ...
        if (st.session_state.samtal_historik 
            ...
            st.session_state.samtal_senast_behandlat = len(st.session_state.samtal_historik) - 1
...
"""
with open("parser/assistent.py", "w", encoding="utf-8") as f:
    f.write(assistent_content)
