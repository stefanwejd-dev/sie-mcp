...
def rendera_kallchip(data_page=None):
    """Visar aktuell datakälla och låter användaren byta i en expander i sidomenyn."""
    import streamlit as st
    
    text = ""
    if "spiris_fel" in st.session_state:
        text = "🔴 Spiris: Autentiseringsfel"
    else:
        if st.session_state.get("spiris_tokens") is not None:
            if st.session_state.get("sie") is not None:
                namn = getattr(st.session_state.sie, "företagsnamn", None) or "Spiris"
                period = st.session_state.get("spiris_dashboard_period")
                period_str = f" • {period[0]}–{period[1]}" if period else ""
                text = f"☁️ Spiris • {namn}{period_str}"
            else:
                text = "🟡 Spiris ansluten (ingen data)"
        else:
            text = "⚪ Ingen källa ansluten"
            
    with st.expander("🔌 Datakälla", expanded=False):
        st.markdown(f"**{text}**")
        if st.button("Byt källa...", width="stretch"):
            if data_page:
